...
public class ClientConnectionTest {
    ...
    public void setUp() throws SQLException, IOException {
        ...
        InetSocketAddress address = new InetSocketAddress(60000);
        ss.bind(address);
    ...
    public void testFullFlow() throws IOException, RemoteSocketClosedException,
        ...
        clientSocket.write(ProtocolMessage.toBytes(new SendMessageRequest(
                "Hey I am a message", (byte) 10, Context.NONE, queueList)));
        while (!m.processRead(listeningSocket));
        clientSocket.write(ProtocolMessage.toBytes(new SendMessageRequest(
                "It feels lonely in here", (byte) 8, Context.NONE, queueList,
                "Mario")));
        while (!m.processRead(listeningSocket));
        clientSocket.write(ProtocolMessage.toBytes(new SendMessageRequest(
                "Hey I am a request", (byte) 5, Context.REQUEST, queueList)));
        while (!m.processRead(listeningSocket));
        Thread.sleep(1000);
        clientSocket.write(ProtocolMessage.toBytes(new SendMessageRequest(
                "Hey I am a response", (byte) 2, Context.RESPONSE, queueList,
                "Mario")));
        while (!m.processRead(listeningSocket));
        ...
        for (Message message : messages) {
            ...
            case 10:
                ...
                assertEquals(message.getContext(), Context.NONE.getByteValue());
            ...
            case 5:
                ...
                assertEquals(message.getContext(),
                        Context.REQUEST.getByteValue());
                assertEquals(message.getReceiver(), null);
                break;
            case 2:
                assertEquals(message.getContent(), "Hey I am a response");
                assertEquals(message.getContext(),
                        Context.RESPONSE.getByteValue());
                assertEquals(message.getReceiver(), "Mario");
                break;
            default:
        ...
        for (int i = 0; i < 14; i++) {
            ...
            case RETURNED_MESSAGES:
                ...
                case 10:
                    ...
                    assertEquals(rmr.getContext(), Context.NONE);
