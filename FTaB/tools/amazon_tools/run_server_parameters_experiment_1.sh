...
do
    ...
    do
        clients=$(($clientsPerWorker*$workers/10))
        for traceFile in 1 2 3 4 5
        do
            offset=$(($clients*($traceFile - 1)))
            sed -i 's/ReaderNumber = .*/ReaderNumber = '"$clients"'/g' $1/config/trace_pilot_clients_${traceFile}_reads_11.11.ini
            sed -i 's/ReaderOffset = .*/ReaderOffset = '"$offset"'/g' $1/config/trace_pilot_clients_${traceFile}_reads_11.11.ini
            sed -i 's/SenderNumber = .*/SenderNumber = '"$clients"'/g' $1/config/trace_pilot_clients_${traceFile}_sends_11.11.ini
            sed -i 's/SenderOffset = .*/SenderOffset = '"$offset"'/g' $1/config/trace_pilot_clients_${traceFile}_sends_11.11.ini
        done
        ...
        $1/tools/amazon_tools/run_experiment_cycle.sh $1/tools/amazon_tools/ ~/logs/log-$clientsPerWorker-$workers-1
    done
done
